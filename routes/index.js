...
// our db model
var GlucoseTemp = require("../models/model.js");
...
router.get('/create-temp', function(req, res) {
  res.render('daily-form.html')
})


router.get('/displayData', function(req, res) {
  res.render("showGlucose.html")
})
...
router.post('/create-temp', function(req, res) {
  ...
  var dataCollection = {
    ...
    state: [{
      type: 'fasted',
      sampleTime: morningtime,
      measurement: morningvalue,
    }, {
      type: 'prelunch',
      sampleTime: prelunchtime,
      measurement: prelunchvalue,
    }, {
      type: 'postlunch',
      sampleTime: postlunchtime,
      measurement: postlunchvalue,
    }, {
      type: 'bedtime',
      sampleTime: bedtime,
      measurement: bedvalue,
    }]
  }
  // Added for debugging by Sam
  // return res.json(dataCollection)

  var glucoseData = new GlucoseTemp(dataCollection)

  glucoseData.save(function(err, data) {
    ...
    console.log('saved a new dataset!');
    console.log(data);
    console.log(dataCollection);

    // now return the json data of the new animal
    //only necessary if being showed in res.json(jsondata)
    var jsonData = {
      status: 'OK',
      data: data
    }
...
//this will reqest all data that we have stored in MongoDatabase
router.get('/showDBdata', function(req, res) {
  console.log("We have a request for all data")

  GlucoseTemp.find(function(err, data) {
...
//       var error = {
//         status: 'ERROR',
//         message: 'Could not find that animal'
//       };
//       return res.json(error);
//     }
//
...
//     status: 'ERROR',
//     message: 'You are missing a required field or have submitted a malformed request.'
//   })
//
